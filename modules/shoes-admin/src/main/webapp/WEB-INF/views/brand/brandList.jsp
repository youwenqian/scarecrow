...
<head>
    ...
    <script type="text/javascript" src="${root}statics/admin/brandList.js"></script>
...
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" id="queryForm">
            ...
            <input type="text" name="name"  placeholder="请输入名称" autocomplete="off" class="layui-input" value="${condition.name}">
            <button class="layui-btn"  lay-submit="" lay-filter="sreach" id="brandSearchBtn"><i class="layui-icon">&#xe615;</i></button>
        </form>
    </div>
    <xblock>
        <button class="layui-btn layui-btn-danger" onclick="delAllBrand()"><i class="layui-icon"></i>批量删除</button>
        <button class="layui-btn" onclick="x_admin_show('添加品牌','${root}brand/toBrandAdd', 600, 400)"><i class="layui-icon"></i>添加</button>
    ...
    <table class="layui-table">
        ...
        <c:forEach items="${brands}" var="brand" varStatus="num">
            <tr class="result">
                <td>
                    <div class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id='${brand.id}'><i
                            class="layui-icon">&#xe605;</i></div>
                </td>
                <td>${brand.id}</td>
                <td>${brand.name}</td>
                <td>${brand.createUser}</td>
                <td>
                    <fmt:formatDate value="${brand.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
                </td>
                <td class="td-manage">
                    <a title="编辑"  onclick="x_admin_show('编辑','../brand/toBrandEdit?id=${brand.id}', 600, 400)" href="javascript:;">
                        <i class="layui-icon">&#xe642;</i>
                    </a>
                    <a title="删除" onclick="delBrand(this,'${brand.id}')" href="javascript:;">
                        <i class="layui-icon">&#xe640;</i>
                    </a>
                </td>
            </tr>
