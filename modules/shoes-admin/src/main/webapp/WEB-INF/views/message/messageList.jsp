...
<head>
    ...
    <script type="text/javascript" src="${root}statics/admin/messageList.js"></script>
...
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" id="queryForm">
            ...
            <input type="text" name="keyword"  placeholder="请输入关键字" autocomplete="off" class="layui-input" value="${keyword}">
            <button class="layui-btn"  lay-submit="" lay-filter="sreach" id="searchBtn"><i class="layui-icon">&#xe615;</i></button>
        </form>
    </div>
    <xblock>
        <button class="layui-btn layui-btn-danger" onclick="delAll()"><i class="layui-icon"></i>批量删除</button>
        <button class="layui-btn" onclick="x_admin_show('添加广告','${root}message/toMessageAdd', 600, 400)"><i class="layui-icon"></i>添加</button>
    ...
    <table class="layui-table">
        ...
        <c:forEach items="${messages}" var="message" varStatus="num">
            <tr class="result">
                <td>
                    <div class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id='${message.id}'><i
                            class="layui-icon">&#xe605;</i></div>
                </td>
                <td>${message.id}</td>
                <td>${message.name}</td>
                <td class="td-manage">
                    <a title="编辑"  onclick="x_admin_show('编辑','${root}message/toMessageEdit?id=${message.id}', 600, 400)" href="javascript:;">
                        <i class="layui-icon">&#xe642;</i>
                    </a>
                    <a title="删除" onclick="del(this,'${message.id}')" href="javascript:;">
                        <i class="layui-icon">&#xe640;</i>
                    </a>
                </td>
            </tr>
